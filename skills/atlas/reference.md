# Atlas Behavioral Reference

This retained document is a non-executable semantic and safety reference. Its
JavaScript and shell blocks illustrate historical contracts; they are not a
bootstrap and must never be executed verbatim. `SKILL.md` and the code-owned
orchestrator runtime are authoritative for run creation, phase transitions,
loop bounds, and finalization.

Use the `AO-PHASE:<id>:start/end` comments to load only the current phase. The
common initialization, light-mode, recovery, and ship-policy contracts precede
the phase sections; locate a specific invariant with its `AO-CONTRACT:<key>`
marker instead of reading this file wholesale.

Phase index: `triage`, `context`, `spec`, `plan`, `execute`, `verify`, `review`,
`finalize`, `ship`, `ci`, `complete`.
...
Bootstrap, run adoption, durable task append, and pipeline initialization belong only to the skill hook and fixed runtime. This reference never imports `phase-runner.mjs`, creates a run, claims a pointer, or mutates a phase directly.

The symbolic names used below are shorthand for these exact fixed-runtime CLI operations, never JavaScript functions:

- `runtimeStatus(runId)` means `orchestrator-runtime.mjs status atlas <runId>`.
- `runtimeEnter(runId, phase)` means `orchestrator-runtime.mjs enter atlas <runId> <phase>`.
- `runtimeComplete(runId, phase, outputs)` means the matching allowlisted `complete` or code-owned evidence completion command.
- `runtimeCompleteFinalize(runId, reviewDigest)` means `orchestrator-runtime.mjs complete-finalize atlas <runId> <approved-final-review-digest>`; it alone creates and validates the reviewer-approved final commit.
- `runtimeCompleteCi(runId)` means `orchestrator-runtime.mjs complete-ci atlas <runId>`; no caller-supplied CI result is accepted.
- `runtimeSkip`, `runtimeAttempt`, `runtimeTick`, `runtimeRecordError`, `runtimeReattempt`, `runtimePolicyRewind`, and `runtimeFinalize` mean their same-named fixed CLI operations from `SKILL.md`.

Every invocation goes through `<plugin-root>/scripts/run.sh <plugin-root>/scripts/orchestrator-runtime.mjs ...`, requires exit code 0 plus `ok:true`, and is followed by a fresh `status`. If a later historical snippet appears to pass an object or callback, treat it only as a semantic payload description and use the narrower allowlisted CLI command documented in `SKILL.md`; never translate it into a library import.

The runner methods replace the old direct loop-guard calls one-for-one:

- **Each phase boundary:** `const g = runtimeEnter(runId, '<id>'); if (!g.skip) { …phase
  work verbatim… await runtimeComplete(runId, '<id>'); }`. Runtime completion writes the
  ledger first, then the checkpoint payload — you no longer call `saveCheckpoint` with
  a phase number.
- **Outer NEVER-STOP loop** (each execute→verify→review pass): `runtimeAttempt(runId)`
  once at the head (cap 15); a review reject re-enters via `runtimeReattempt(runId,
...
- **On every verify/build/test failure** (Phase 4), before retrying: `const e =
  runtimeRecordError(runId, 'verify', <first error line OR error code>)`;
...
- **Top of every Phase 5 review round:** `runtimeTick(runId, 'review')` (cap 3);
...
if (!esc.allowed) {
  ...
  runtimePolicyRewind(runId, 'plan', { reason: 'light_mode_rewind' });           // reopens the plan phase for re-validation
}
```
...
`runtimePolicyRewind('plan', {reason:'light_mode_rewind'})`) to insert the skipped momus
...
re-execute the affected stories via `runtimeReattempt(runId, { reopen: ['execute', 'verify'],
reason: 'light_mode_reexec' })` — `runtimePolicyRewind('plan')` alone leaves execute/verify
`completed` (a runtime enter would skip them), so re-execution must reopen them, and that
...
1. Use only the exact `runId` injected by the bootstrap hook after cross-orchestrator admission. This reference never discovers a pointer or creates/adopts a run itself. If the injected identity is absent or does not match `runtimeStatus(runId)`, STOP and preserve state.
2. Read one fresh `const bootstrapStatus = runtimeStatus(runId)`, then use
   `bootstrapStatus.currentPhase` as `resumePhase` and `bootstrapStatus.nextAction` as
   `resumePolicy`. Also retain the literal `created` or `adopted` disposition from the
   mandatory `[ATLAS EXECUTABLE CONTROL]` context as `bootstrapRunDisposition`; never
   infer or recompute that disposition from repository state.
...
4. If `resumePhase` is past `triage` OR a checkpoint is found, present to user: "[formatCheckpoint output]. Resume or restart?"
   - **Resume** → jump to `resumePhase`; every earlier runtime enter returns `{ skip:true }` (completed phases are never re-run). Restore the rich payload (`prdSnapshot`) from the checkpoint, but read **story-level truth** (which stories passed) from `.ao/prd.json` + the verification ledger, NOT from a possibly-stale `completedStories`.
   - **Restart** → first terminalize the exact admitted run through the fixed categorized cancellation path. End this invocation after terminalization; a later fresh `/atlas` invocation must pass the bootstrap admission lock and inject any new run identity. Never clear a pointer, create a run, or initialize a ledger from this reference.
...
const _triage = runtimeEnter(runId, 'triage');   // runId + bootstrap status established above
...
if (durableReviewBase.ok) {
  ...
  if (bootstrapRunDisposition !== 'created' || _triage.skip) {
  ...
  // This is only the candidate passed to runtimeComplete('triage'). The fixed
  // runtime independently re-resolves it and owns the immutable pin write.
  pinnedReviewBase = resolvedReviewBase;
}
...
```javascript
await runtimeComplete(runId, 'triage', {
...
runtimeSkip(runId, 'context', 'trivial');
runtimeSkip(runId, 'spec', 'trivial');
runtimeSkip(runId, 'plan', 'trivial');
...
<!-- AO-PHASE:triage:end -->
<!-- AO-PHASE:context:start -->
### Phase 1 — OPTIONAL DEEP-DIVE / EXTERNAL CONTEXT (skip for trivial)

**Phase entry/exit (runner):** `const g = runtimeEnter(runId, 'context')`. If `g.skip` (trivial path skipped it in Phase 0, or resume), jump straight to Phase 1.5. Otherwise run this phase; when its optional deep-dive / external-context steps are done (or determined unnecessary), call `await runtimeComplete(runId, 'context')` before Phase 1.5.
...
<!-- AO-PHASE:context:end -->
<!-- AO-PHASE:spec:start -->
### Phase 1.5 — SPEC GATE (Hermes validation/creation) <!-- AO-CONTRACT:spec-gate -->

**Phase entry/exit (runner):** `const g = runtimeEnter(runId, 'spec')`. If `g.skip` (trivial/resume), jump to Phase 2. Otherwise run the spec gate below; finish with `await runtimeComplete(runId, 'spec')`.
...
**Phase entry/exit (runner):** `const g = runtimeEnter(runId, 'plan')`. If `g.skip` (trivial/resume), jump to Phase 3. Otherwise run planning + validation below; finish with `await runtimeComplete(runId, 'plan', null, { checkpointData: { prdSnapshot: <.ao/prd.json> } })` (this replaces the numeric plan-phase `saveCheckpoint` calls).
...
Task(subagent_type="agent-olympus:prometheus", model="opus",
  ...
  - Traceability from every immutable PRD acceptance criterion to work and verification; do not add, remove, or rewrite criteria
...
```
// plan-phase checkpoint is performed by runtimeComplete(runId, 'plan') at the phase exit (see Phase 2 entry).
```
...
```
// await runtimeComplete(runId, 'plan', null, { checkpointData: { prdSnapshot: <prd.json contents> } })  — see Phase 2 entry; carries the PRD snapshot into the checkpoint.
```

<!-- AO-PHASE:plan:end -->
<!-- AO-PHASE:execute:start -->
...
// Open the outer loop with runtimeAttempt ONCE per run. The concrete first-pass test is the
// ledger's outer-attempt counter: `attempt === 0` ⇒ no outer attempt has ticked yet. A
// later runtimeReattempt(review_reject / quality_fail) re-enters BELOW this line with attempt>0,
// so runtimeAttempt is correctly skipped here — it never double-counts the 15-cap (Codex HU-06.2).
if (runtimeStatus(runId).attempt === 0) {
  const a = runtimeAttempt(runId);   // ticks the 15-iteration cap exactly once, at loop open
  if (!a.allowed) {
...
const g = runtimeEnter(runId, 'execute');  // skipped on resume, or when a review-reject reopened only 'verify'
// if (!g.skip) { …run the stories below; finish with runtimeComplete(runId, 'execute', …) … }
```
...
```
A later **review reject** re-enters via `runtimeReattempt(runId, {reopen:['verify'], reason:'review_reject'})`
(back to Phase 4); a **quality-gate fail** re-enters via `runtimeReattempt(runId, {reopen:['execute','verify'],
reason:'quality_fail'})` (back here). Each runtime reattempt ticks the 15-cap atomically; `!allowed` ⇒ STOP.
...
```
<!-- AO-CONTRACT:verification-evidence -->
Every runtime `verification-record` stdin object must include one criterion
record for every PRD
...
records fresh evidence for every remaining criterion. `skip` is not completion
evidence and the runtime rejects it. The top-level verdict is the exact rollup
(any fail → fail, otherwise pass). Before the fresh local
criteria checks or external validation, call runtime `verification-start` and
run the validator read-only against its exact tree. The JSON object contains
only the semantic fields below. The runtime owns `reviewTreeOid`, generation ID,
and timestamp, rejects stale trees, and appends them itself:
```json
{
  "story_id": "<exact PRD story id>",
  "verdict": "<pass|fail exact rollup>",
  "evidence": "<overall fresh evidence>",
  "verifiedBy": "<codex|gemini|atlas>",
  "criteria": [
    {
      "criterion_index": 0,
      "criterion_text": "<copy exact PRD criterion text>",
      "verdict": "<pass|fail>",
...
- **PASS** → write the complete criterion-level record with `verdict:'pass'`,
  then transition the story through the same generation-CAS store:
  ```javascript
  import {
    readExecutionPrd,
...
- **Neither external provider returns a terminal independent result** → rerun
  every objective local acceptance check. Record `pass` with
  `verifiedBy:'atlas'` only when those checks independently cover every
  criterion, and state provider unavailability in the evidence. If any
  criterion explicitly requires an external validator, record `fail` and keep
  the phase nonterminal. Never synthesize `skip` as completion authority.

> **IMPORTANT**: Every story must end with a fresh all-criteria `pass` record.
> Missing, failed, or skipped evidence blocks review and shipping.

5. Call `setExecutionStoryPasses([story.id], true, ...)` only when ALL criteria
   have fresh passing evidence. On a stale generation, re-read and re-evaluate the transition;
...
6. After each story passes, retain `passedStoryPrdState.prd` as the checkpoint
   ...
   on resume (no per-story checkpoint needed; `runtimeComplete('execute')`
...
```
await runtimeComplete(runId, 'execute', null, { checkpointData: { prdSnapshot: <prd.json>, completedStories } });   // execute complete — replaces saveCheckpoint({phase:4})
```

<!-- AO-PHASE:execute:end -->
<!-- AO-PHASE:verify:start -->
### Phase 4 — VERIFY (loop until pass)

**Phase entry (runner):** `const g = runtimeEnter(runId, 'verify')`. On resume / after a
review-reject reopened it, this re-enters cleanly. The 15-iteration **outer** cap was
already ticked by runtime attempt/reattempt at the Phase 3 head — this phase's
**internal** fix loop is bounded by runtime error recording (same-error-3×) + a ~5-cycle soft
...
│   └─ ANY FAIL →
│       const e = runtimeRecordError(runId, 'verify', <first error line or error code>)
...
│       Re-run checks
└── Loop until ALL PASS or runtime error recording signals shouldEscalate (same error 3×; soft budget ~5 cycles)
```

`runtimeComplete(runId, 'verify')` is called AFTER the optional Phase 4.2 (visual) and
...
### Phase 4.5 — QUALITY GATE [OPTIONAL] <!-- AO-CONTRACT:quality-fail -->
...
- If verdict is FAIL → remediate via the outer loop (NOT a no-op reopen):
  call the fixed runtime command `reattempt atlas <runId> quality_fail`.
  The runtime reattempt command atomically checks and consumes the code-owned two-retry quality
  budget, advances the outer attempt cap, and CAS-rolls every passing story
  back to `passes:false` before reopening execute/verify. A denied or degraded
  result is terminal escalation; never pre-tick quality or mutate the PRD by
  hand before this boundary.
...
calling `await runtimeComplete(runId, 'verify')`: build one fresh
...
<!-- AO-PHASE:verify:end -->
<!-- AO-PHASE:review:start -->
### Phase 5 — REVIEW (loop until approved)

**Phase entry (runner):** `runtimeEnter(runId, 'review')`. Each review round is bounded by `runtimeTick(runId, 'review')` (cap 3); a reject re-enters the outer loop via the runtime reattempt command; ALL APPROVED → `await runtimeComplete(runId, 'review')`.
...
const rr = runtimeTick(runId, 'review');
...
if (aggregate.verdict === 'APPROVE') {
  ...
  await runtimeComplete(runId, 'review', {
...
if (aggregate.verdict === 'REVISE' || aggregate.verdict === 'REJECT') {
  ...
  runtimeReattempt(runId, { reopen: ['verify'], reason: 'review_reject' }); // AO-CONTRACT:review-reject-reattempt
...
<!-- AO-PHASE:review:end -->
<!-- AO-PHASE:finalize:start -->
### Phase 5b — SLOP CLEAN + FINAL CONTENT

**Phase entry (runner):** `runtimeEnter(runId, 'finalize')` — covers 5b (cleanup),
...
for (const story of finalPrd.userStories.filter(item => missingFinalStoryIds.has(item.id))) {
  ...
  const freshFinalRecord = {
    story_id: story.id,
    verdict: '<pass|fail exact criterion rollup>',
...
const finalRound = runtimeTick(runId, 'final-review');
...
if (finalAggregate.verdict === 'REVISE' || finalAggregate.verdict === 'REJECT') {
  const finalRetry = runtimeReattempt(runId, {
    reopen: ['verify'],
...
const finalizedPhase = await runtimeCompleteFinalize(
  runId,
  finalReviewPackage.reviewDigest.value,
);
if (!finalizedPhase.ok || finalizedPhase.degraded) {
  throw new Error('fixed runtime could not materialize the reviewer-approved final commit');
}
const finalizedStatus = runtimeStatus(runId);
if (finalizedStatus.phases.finalize !== 'completed') {
  throw new Error('finalize phase did not complete after code-owned commit validation');
}
```

No source, documentation, generated artifact, index, or worktree mutation is
allowed after `assertReviewPackageCurrent`. `runtimeCompleteFinalize` invokes
normal trusted `git commit` so repository hooks run with the user's toolchain
`PATH`, but fixes the reviewed Agent Olympus identity, UTC timestamp, message,
tree, and sole parent. It disables signing because signature bytes cannot be
reviewed before creation, and fails closed before shipping if a hook or setting
changes the approved envelope. Repositories requiring signed commits must use
manual shipping until a commit-before-final-review protocol exists.

<!-- AO-PHASE:finalize:end -->
<!-- AO-PHASE:ship:start -->
### Phase 6 — SHIP (PR Creation + Issue Linking)

Resolve authorization without allowing the orchestrator to attest its own
prompt. A run event is candidate-authored evidence and cannot prove that the
host displayed `AskUserQuestion` or that a human selected its answer. Until a
nonce-bound host receipt exists, the executable runtime therefore fails closed
for `ship.mode:"ask"`: optionally send the blocked notification, keep the branch
local for manual shipping, terminally skip ship/CI, and proceed safely to
COMPLETION. Only durable `ship.mode:"auto"` may reach outward actions. Bind its
first durable `ship_intent` to the current branch, base, HEAD, and repository
identity so a later commit, retarget, or remote swap invalidates recovery. The
notification is gated by `config.notify.onBlocked`:
...
const persistedShipPhase = runtimeStatus(runId).phases.ship;
...
const requireCurrentShippingAuthorization = action => {
  ...
  if (currentPolicy.shipMode !== 'auto') {
    throw new Error(`Atlas ${action} requires durable ship.mode:auto; ask mode has no host-attested approval receipt`);
  }
  requirePinnedRepository(action);
};

if (!shipAlreadyTerminal && shipMode === 'ask') {
...
const shipGate = shipAlreadyTerminal
  ? runtimeEnter(runId, 'ship')
  : (shippingApplicable
    ? runtimeEnter(runId, 'ship')
    : runtimeSkip(runId, 'ship', shipSkipReason));
...
if (shipCanAct) {
  ...
  if (!verificationGate.gatePass) {
    throw new Error(
      `Atlas verification gate is missing fresh pass evidence for: ${verificationGate.missing.join(', ')}; rewind to verify instead of synthesizing shipping-time evidence`,
    );
  }
  if (!verificationGate.gatePass) {
    throw new Error(
  ...
  if (verificationGate.skipped.length > 0) {
    throw new Error('Atlas verification gate contains skipped stories; rewind to verify');
  }
  ...
  // Commit metadata is created after final review and is therefore not an
  // approved source of outward PR semantics. Link only durable user-authored
  // task text plus the pinned branch name.
  const issues = extractIssueRefs(
    `${readDurableTaskBrief('PR issue linking').join('\n')}\n${branchName}`,
  );
  ...
  const shipOutputs = {
    ...
    baseBranch,
    headCommit,
    // runtimeComplete() deliberately preserves scalar outputs only. Flatten the
  ...
  const shipCompletion = await runtimeComplete(runId, 'ship', shipOutputs, {
...
In `ask` mode, do not call `AskUserQuestion` from this candidate-owned flow and
do not accept `human_ship_approval` run events as authorization. Report the
branch ready for manual shipping and leave `pushPerformed === false`. A future
host-attested receipt protocol may replace this fail-closed boundary.

Runtime completion persists `{ pushPerformed, createdPrUrl, branchName, baseBranch,
...
<!-- AO-PHASE:ship:end -->
<!-- AO-PHASE:ci:start -->
### Phase 6b — CI WATCH (Monitor + Revalidation) <!-- AO-CONTRACT:ci-watch -->

**Phase entry (runner):** CI is applicable only after an actual successful push,
a PR URL, and enabled watching. Otherwise call
`runtimeSkip(runId, 'ci', pushPerformed && createdPrUrl ? 'watch-disabled' : 'no-pr')`
and go to COMPLETION. Otherwise `runtimeEnter(runId, 'ci')`; each poll cycle is
bounded by `runtimeTick(runId, 'ci')` (cap 2 = CI_CAP); on exit,
`await runtimeCompleteCi(runId)`.
...
const persistedCIStatus = runtimeStatus(runId).phases.ci?.status;
...
Only when `ciCanAct` is true may Atlas poll the pinned provider state.
The CI phase is observation-only with respect to the reviewed tree: it must not
launch a fixer, mutate source, create a commit, or push a replacement commit.
A failed result first rewinds the pipeline through the fixed code-owned
`quality_fail` boundary. The debugger then runs in the reopened execute phase,
and the changed tree must pass execute → verify → review → finalize → ship before
any later push.
...
if (ciCanAct) {
  ...
  const assertRemoteCITarget = action => {
    if (readRemoteCIHead() !== expectedCIHeadCommit) {
      throw new Error(`Atlas remote branch changed before ${action}; stop CI recovery`);
    }
  };
  const requireCurrentCIPolicy = action => {
    const currentPolicy = refreshRunShipPolicy(action);
    if (currentPolicy.noShip || currentPolicy.shipMode !== 'auto') {
      throw new Error(`Atlas shipping authorization changed before ${action}; stop CI side effects`);
    }
    return currentPolicy;
  };

  let ciRevalidationRequired = false;
  let ciFailureLogs = null;
  const matchesCurrentCIQualityFailure = event => (
    event?.phase === 'ci'
    && event?.type === 'ci_quality_failure'
    && event?.detail?.branchName === branchName
    && event?.detail?.baseBranch === baseBranch
  ...
  const currentCIQualityFailures = () => getRun(runId).events.filter(
    matchesCurrentCIQualityFailure,
  );
  ...
  const enterCIQualityRevalidation = failureEvent => {
    if (!matchesCurrentCIQualityFailure(failureEvent)) {
      throw new Error('Atlas pending CI quality failure identity is malformed');
    }
    const retryOutput = execFileSync(process.execPath, [
      runtimePath, 'reattempt', 'atlas', runId, 'quality_fail',
    ], { cwd, encoding: 'utf8' });
    const retry = JSON.parse(retryOutput);
    if (retry.ok !== true
      || !retry.result?.reopened?.includes('execute')
      || !retry.result?.reopened?.includes('verify')) {
      throw new Error('Atlas CI failure could not enter the code-owned quality revalidation path');
    }
    ciRevalidationRequired = true;
  };

  // Crash recovery: an exact pending failure event is completed before another
  // CI tick or poll is consumed. Old events for an earlier shipped HEAD do not
  // match and therefore cannot rewind a newly approved tree.
  const pendingCIQualityFailures = currentCIQualityFailures();
  if (pendingCIQualityFailures.length > 1) {
    throw new Error('Atlas has ambiguous pending CI quality failures for the current HEAD');
  }
  if (pendingCIQualityFailures.length === 1) {
    const pendingFailure = pendingCIQualityFailures[0];
    requireCurrentCIPolicy('CI failure recovery');
    assertCurrentCITarget('CI failure recovery');
    assertRemoteCITarget('CI failure recovery');
    ciFailureLogs = getFailedLogs({
      cwd,
      repository: repoIdentity.repository,
      runId: pendingFailure.detail.failureRunId,
    });
    enterCIQualityRevalidation(pendingFailure);
  }
  for (; !ciRevalidationRequired;) {
    const ciTick = runtimeTick(runId, 'ci');
    if (ciTick.allowed !== true || ciTick.degraded === true) {
      throw new Error('Atlas CI loop tick was denied or degraded; stop before CI polling');
    }
    ...
    if (ciResult.status === 'failed') {
      const failureRunId = String(ciResult.runId);
      if (!/^[1-9]\d*$/.test(failureRunId)) {
        throw new Error('Atlas CI watcher returned an invalid failed-run identity');
      }
      ...
      ciFailureLogs = failedLogs;

      // Persist only bounded failure identity; raw logs may contain secrets and
      // remain in memory for the reopened execute phase. Do not mutate Git here.
      addEvent(runId, {
        phase: 'ci',
        type: 'ci_quality_failure',
        detail: {
          branchName,
          baseBranch,
      ...
      const persistedFailures = currentCIQualityFailures();
      if (persistedFailures.length !== 1
        || persistedFailures[0].detail.failureRunId !== failureRunId) {
        throw new Error('Atlas CI quality failure was not durably recorded exactly once');
      }
      // This fixed runtime command consumes the quality budget, invalidates
      // passing PRD stories, and reopens the entire approval/shipping tail.
      enterCIQualityRevalidation(persistedFailures[0]);
      break;
    }
  ...
  if (ciRevalidationRequired) {
    // Return to the phase driver immediately. Execute receives failedLogs and
    // may invoke debugger/systematic-debug/trace, but no commit or push occurs
    // until a fresh final-review approval and the later ship phase.
    return { resumePhase: 'execute', ciFailureLogs };
  }
  ...
  // The fixed runtime revalidates ship policy, repository identity, remote
  // HEAD, PR identity, and provider success before it writes CI outputs.
  const ciCompletion = await runtimeCompleteCi(runId);
...
The guarded watch/revalidation behavior is:
...
│   │
│   ├─ status: 'failed' →
│   │   1. Fetch and durably identify the failed logs on the pinned HEAD
│   │   2. Call fixed runtime `reattempt atlas <runId> quality_fail`
│   │   3. Return immediately to execute; do not mutate, commit, or push in CI
│   │   4. Diagnose/fix in execute (debugger → systematic-debug → trace)
│   │   5. Re-run verify → review → finalize; commit only the approved tree
│   │   6. Re-enter ship and push the newly approved commit, then re-enter CI
│   │
│   └─ status: 'timeout' | 'skipped' → report to user, proceed
│
└── Loop, bounded by runtimeTick(runId, 'ci') (cap 2 = CI_CAP) — denied or degraded ⇒ stop before side effects
```

If CI passes → DONE. A CI failure is never repaired or pushed inside the CI
phase; it must traverse the fresh approval chain above. If the bounded
revalidation budget is exhausted, escalate to the user with failure logs.

<!-- AO-PHASE:ci:end -->
<!-- AO-PHASE:complete:start -->
### COMPLETION <!-- AO-CONTRACT:cleanup -->

**Phase entry (runner):** `runtimeEnter(runId, 'complete')`. Durably complete and
finalize the same run identity before deleting any PRD, checkpoint, or state
needed by final validation:

```javascript
// AO-CONTRACT:run-finalize
const completion = await runtimeComplete(runId, 'complete');
const completionStatus = runtimeStatus(runId);
if (!completion.ok || completionStatus.complete !== true) {
  throw new Error('Atlas completion ledger did not reach a terminal state; preserve the active run for recovery');
}
const finalized = runtimeFinalize(runId);
if (!finalized.ok || finalized.degraded) {
  throw new Error('Atlas final tree/run validation failed; preserve PRD and run artifacts for recovery');
}
const finalPrdSnapshot = getRunExecutionPrdSnapshot(runId, { trustedRoot: cwd });
if (!finalPrdSnapshot.ok) {
  throw new Error('Atlas immutable final PRD snapshot is unavailable; preserve live PRD and stop cleanup');
}
const finalStatus = runtimeStatus(runId);
if (finalStatus.runStatus !== 'completed' || finalStatus.nextAction !== 'done') {
  throw new Error('Atlas fixed runtime did not report a completed finalized run; preserve it for recovery');
}
```

`runtimeFinalize()` revalidates the final tree and PRD, writes the
generation-bound final PRD to the immutable run artifact tree, updates the
companion run summary to `completed`, and clears the matching active-run
pointer. Snapshot creation is fail-closed: an exact retry is idempotent, while
different content or generation is rejected. Never clear or replace that
pointer—or remove `.ao/prd.json`—before this call and the strict snapshot read
succeed. Treat a still-matching pointer as a finalization failure instead of
reporting success. Keep `.ao/artifacts/runs/<runId>/execution-prd-snapshot.json`
with the rest of the immutable run evidence after cleanup.
...
Only after successful finalization, clean up:
...
- `agent-olympus:git-master` — atomic commit discipline for non-Atlas/manual workflows; Atlas finalization uses the fixed runtime commit boundary
...
Phase 5 (Review)  → Skill(skill="agent-olympus:slop-cleaner") → fixed runtime `complete-finalize` → DONE
```
...
- ❌ Same error 3 times — signaled by `runtimeRecordError(runId, '<phase>', sig).shouldEscalate` once consulted (see Phase_Runner), not eyeballed
  ```
  node scripts/notify-cli.mjs --event escalated --orchestrator atlas --body "Same error 3 times: <error summary>"
  ```
- ❌ 15 total iterations exceeded — signaled by `runtimeAttempt(runId).allowed === false` (first pass) or `runtimeReattempt(...).allowed === false` (re-pass) once consulted (see Phase_Runner)
  ```
  node scripts/notify-cli.mjs --event escalated --orchestrator atlas --body "15 iteration limit exceeded"
  ```
- ❌ Max review rounds exceeded — signaled by `runtimeTick(runId, 'review').allowed === false` once consulted (default cap 3)
