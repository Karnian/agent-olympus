...
- Hook runtime contract: Claude Code sends JSON on stdin and expects JSON on stdout. Hooks must fail open (`catch` -> `{}` -> exit 0), avoid `console.log`, and respect Claude Code hook timeouts.
