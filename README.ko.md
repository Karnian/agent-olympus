...
2개의 오케스트레이터, 18개의 전문 에이전트, 26개의 워크플로우 스킬. npm 의존성 제로.
...
- **26개 워크플로우 스킬**: atlas, athena, plan, ask, deep-interview, research, trace, cancel, slop-cleaner, git-master, deepinit, deep-dive, consensus-plan, external-context, verify-coverage, tdd, systematic-debug, brainstorm, finish-branch, **design-critique, a11y-audit, design-system-audit, ux-copy-review, ui-review**, harness-init, sessions
...
- **627개+ 단위 테스트**: `node:test` 기반 40개 파일의 종합 테스트 스위트
...
`node:test` 기반 테스트 스위트 (40개 파일, 627개+ 테스트)가 핵심 훅 라이브러리를 커버합니다:
