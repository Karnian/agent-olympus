...
- **Athena** — 피어-투-피어 팀 오케스트레이션. 여러 에이전트가 SendMessage로 서로 직접 협력하고, Codex/Gemini 워커는 어댑터 시스템(codex-appserver > codex-exec > tmux 폴백)을 통해 실행. 실시간 조율이 필요한 상호의존적 작업에 최적.
...
- **Native Teams 설정 폴백** *(v0.9.8)*: `.ao/autonomy.json`의 `nativeTeams: true`로 `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` 환경변수 없이도 Native Agent Teams 활성화
...
- **변경 인지 리뷰 라우터** *(v1.0.2, [gstack](https://github.com/garrytan/gstack) 차용)*: CSS-only diff는 `{aphrodite, designer}`만 호출 (리뷰어 오버헤드 60-80% 절감), 30+ 정규식 보안 패턴은 `security-reviewer` 강제 포함, `alwaysInclude:["*"]` 롤백 경로 제공
