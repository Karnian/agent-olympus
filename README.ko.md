...
Agent Olympus는 Claude Code를 위한 독립형 플러그인으로, 두 가지 자율 주행 AI 오케스트레이터를 제공합니다. 이 플러그인을 통해 복잡한 개발 작업을 15개의 전문화된 에이전트, 10개의 워크플로우 스킬, Claude + Codex 멀티 모델 실행, tmux 기반 팀 인프라를 활용하여 완전히 자동화된 방식으로 처리할 수 있습니다.
...
- **10개 워크플로우 스킬** — /atlas, /athena, /ask, /deep-interview, /deepinit, /research, /trace, /slop-cleaner, /git-master, /cancel
...
agent-olympus/
├── .claude-plugin/
│   ├── plugin.json          — 플러그인 manifest (v0.2.0)
