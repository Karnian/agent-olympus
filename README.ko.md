...
2개의 오케스트레이터, 19개의 전문 에이전트, 35개의 워크플로우 스킬. npm 의존성 제로.
...
- **35개 워크플로우 스킬**: atlas, athena, plan, ask, deep-interview, research, trace, cancel, slop-cleaner, git-master, deepinit, deep-dive, consensus-plan, external-context, verify-coverage, tdd, systematic-debug, brainstorm, finish-branch, **design-critique, a11y-audit, design-system-audit, ux-copy-review, ui-review**, harness-init, sessions, **teach-design, normalize, polish, typeset, arrange, taste, ui-remediate, resume-handoff** *(v1.0.2 신규)*, **setup-gemini-auth** *(v1.1.3 신규)*
...
- **2000개+ 단위 테스트**: `node:test` 기반 77개 파일의 종합 테스트 스위트 (v1.1.4: 2012 통과)
...
`node:test` 기반 테스트 스위트 (77개 파일, 2012개+ 테스트, v1.1.4 기준)가 핵심 훅 라이브러리를 커버합니다:
