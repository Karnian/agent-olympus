...
- **1300개+ 단위 테스트**: `node:test` 기반 64개 파일의 종합 테스트 스위트 (v1.0.2: 1326 통과)
...
`<approval-flag>`는 Claude의 권한 수준에서 자동 결정됩니다 ([권한 미러링](#features) 참조). `.ao/autonomy.json`의 `codex.approval`로 오버라이드 가능.
...
`node:test` 기반 테스트 스위트 (64개 파일, 1326개+ 테스트, v1.0.2 기준)가 핵심 훅 라이브러리를 커버합니다:
