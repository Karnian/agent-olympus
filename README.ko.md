...
2개의 오케스트레이터, 17개의 전문 에이전트, 19개의 워크플로우 스킬. npm 의존성 제로.
...
- **17개 전문 에이전트**: 분석가(Metis), 전략가(Prometheus), 검증자(Momus), 기획자(Hermes), 개발자(Executor), 디자이너, 테스트 엔지니어, 디버거, 아키텍트, 보안 검수자, 코드 리뷰어, 작가(Writer), 탐색가(Explore), Hephaestus, **Themis(품질 게이트)**, Atlas, Athena
- **19개 워크플로우 스킬**: atlas, athena, plan, ask, deep-interview, research, trace, cancel, slop-cleaner, git-master, deepinit, deep-dive, consensus-plan, external-context, verify-coverage, **tdd, systematic-debug, brainstorm, finish-branch**
...
- **295개+ 단위 테스트**: `node:test` 기반 19개 파일의 종합 테스트 스위트
